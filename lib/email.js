...
// =============================================================================
export async function sendLeadRecovery({ to, quiz = {} }) {
  if (!resend) return { skipped: true };
  ...
  const subject  = `Your custom song is waiting… 🎵`;
  const preview  = `You were so close! Your personalized song for ${recipient} is ready to be created.`;
  ...
  const body = `
    ...
    <p style="margin:0 0 20px;font-size:16px;line-height:1.6;color:${C.inkSoft}">
      We noticed you started building a custom song for <strong>${escape(recipient)}</strong>${occasion ? ` for their <strong>${escape(occasion)}</strong>` : ''} — and didn't finish.
    </p>
    ...
    <p style="margin:0 0 20px;font-size:15px;line-height:1.6;color:${C.inkSoft}">
      Your details are saved. It only takes 30 seconds to complete your order and get your song delivered in <strong>6 hours</strong>.
    </p>

    <div style="margin:0 0 24px;text-align:center">
      ${btn(shopUrl, '🎶 Complete my order →')}
    </div>

    <div style="background:${C.creamDark};border-radius:12px;padding:16px 20px;margin:0 0 8px">
      <p style="margin:0;font-size:14px;line-height:1.6;color:${C.inkSoft};text-align:center">
        ⚡ Delivered in <strong>6 hours</strong> &nbsp;·&nbsp; 🔒 Secure checkout &nbsp;·&nbsp; ↩️ 30-day refund
      </p>
    </div>
  `;
  ...
  return resend.emails.send({
    ...
    to,
    subject,
    html: shell({ title: 'Your song is waiting', preview, body })
  });
}
